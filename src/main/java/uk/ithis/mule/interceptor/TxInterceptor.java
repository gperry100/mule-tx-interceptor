...
//        event.getMessage().getOutboundPropertyNames().forEach(prop -> {
//            LOG.error("1Prop is {}", prop);
//        });
//
//        event.getMessage().getInboundPropertyNames().forEach(prop -> {
//            LOG.error("2Prop is {}", prop);
//        });
//
//        event.getMessage().getInvocationPropertyNames().forEach(prop -> {
//            LOG.error("3Prop is {}", prop);
//        });
        String correlationId = event.getMessage().getCorrelationId();
        if (StringUtils.isBlank(correlationId)) {
            correlationId = UUID.randomUUID().toString();
